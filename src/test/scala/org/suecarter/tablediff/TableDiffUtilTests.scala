...
""")).map(x => (stringToReportSection(x._1), stringToReportSection(x._2))) ++ L(
    ...
    cases.foreach {
      case (l, r) =>
        ...
        assert(
          diffReportToString(ReportContent(rowHeaders = diffed(l))) ===
...
""")
    ...
    cases.foreach {
      case (l, r) =>
        ...
        assert(
          diffReportToString(removeDuplicateReportHeaders(fillForwardReportHeaders(diffed(l)))) === diffReportToString(
            diffed(l)
          )
        )
    }
  ...
  test("Fill forward on diff only results") {
    ...
    val cases: List[
      (
          ...
          ReportContent[String, String, String],
          ReportContent[ValueDiff[String], ValueDiff[String], ValueDiff[String]]
      ),
    ] = List(
      (
        """
...
""",
        ReportContent(
          List(List(Right(Some("a")), Right(Some("b")))),
...
""",
        ReportContent(
          List(),
          List(List(Left(EitherSide(Some("a"), Some("x")))), List(Right(Some("c")))),
...
""",
        ReportContent(
          List(List(Right(Some("a")), Left(EitherSide(Some(" "), Some("x"))))),
          List(),
          List(),
          List(List(Right(Some(" ")), Right(None)), List(Right(Some("n")), Right(Some("m"))))
        )
...
object TableDiffUtilTests {
  def diffCell(x: Any): ValueDiff[Any] =
    Right(x.toString.trim match {
      case "" => None
      case y  => Some(y)
    })
  def diffed(x: ReportSection[Any]): ReportSection[ValueDiff[Any]] = x.map(_.map(diffCell))
  def diffed(x: ReportContent[Any, Any, Any]) =
    x.copy(
      rowHeaders = x.rowHeaders.map(_.map(diffCell)),
