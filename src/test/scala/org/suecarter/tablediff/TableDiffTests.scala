...
object TableDiffTestCases {
  ...
  val extraColReport = ReportContent(
    L(L("a", "d"), L("b", "d")),
    L(L("Col1", "Col2", "Col2.5", "Col3"), L("one", "two", "two.5", "three")),
    L(L(7, 5, 2, 2), L(33, 1, 2, 34))
  )
  val extraRowReport = ReportContent(
    L(L("a", "d"), L("z", "y"), L("b", "d")),
    L(L("Col1", "Col2", "Col3"), L("one", "two", "three")),
    L(L(7, 5, 2), L(2, 3, 4), L(33, 1, 34))
  )
  val changeColumnAndRowHeaderReport = ReportContent(
    L(L("a", "d"), L("b", "z")),
    L(L("Col1", "Col2", "Colz"), L("one", "two", "three")),
    L(L(7, 5, 2), L(33, 1, 34))
  )
  val rightReport = ReportContent(
    L(L("a", "d"), L("b1", "d")),
    L(L("Col1", "Col2", "Col4"), L("one", "two", "three")),
    L(L(8, 5, 23), L(33, 1, 34))
  )
  val leftReportWithASmallNumericDiff = ReportContent[String, String, Double](
    L(L("a", "d"), L("b", "d")),
    L(L("Col1", "Col2", "Col3"), L("one", "two", "three")),
    L(L(7, 5, 2), L(33, 1, 34.000001))
  )
  ...
  val unexpectedShapedReport =
    ReportContent(L(L("a"), L("b", "d", "c")), L(L("Col1", "Col2"), L("one")), L(L(7, 5, 2), L(33)))
  val emptyTestReport = ReportContent[String, String, Int](L(), L(), L())
}

class TableDiffTests extends AnyFunSuite {
  test("matching sequences") {
    val cases: List[(String, String, String, List[DiffLocation[Char]], Option[String])] =
      L(
        ("case 1", "a", "b", L(DL('b', n, S(0)), DL('a', S(0), n)), Some("[-a-]{+b+}")),
        ("case 2", "a", "a", L(DL('a', S(0), S(0))), Some("a")),
        ("case 3", "ab", "a", L(DL('a', S(0), S(0)), DL('b', S(1), n)), Some("a[-b-]")),
        ("case 4", "ab", "ac", L(DL('a', S(0), S(0)), DL('c', n, S(1)), DL('b', S(1), n)), Some("a[-b-]{+c+}")),
        (
          "case 5",
          "abd",
          "acd",
          L(DL('a', S(0), S(0)), DL('c', n, S(1)), DL('b', S(1), n), DL('d', S(2), S(2))),
          Some("a[-b-]{+c+}d")
        ),
        (
          "case 6",
          "ab",
          "acd",
          L(DL('a', S(0), S(0)), DL('c', n, S(1)), DL('b', S(1), n), DL('d', n, S(2))),
          Some("a[-b-]{+cd+}")
        ),
        (
          "case 7",
        ...
        ("case 8", "", "", L(), Some("")),
        ("case 9", "a", "", L(DL('a', S(0), n)), Some("[-a-]")),
        ("case 10", "", "a", L(DL('a', n, S(0))), Some("{+a+}")),
        (
          "case 10",
          "ghij",
          "abcd",
        ...
        L(
          ...
          ("2 completely different seqs", xs('a'), xs('b'), xLeftDiffs('a') ++ xRightDiffs('b')),
          ("2 identical seqs", xs('a') ++ xs('b'), xs('a') ++ xs('b'), (xs('a') ++ xs('b')).zipWithIndex.map {
            case (dx, ix) => DL(dx, S(ix), S(ix))
          }),
          (
    ...
    cases.foreach {
      case (description, left, right, expected, expectedStringRep) =>
        ...
        assert(
          leftDiffs.mkString("\n") === rightDiffs.mkString("\n"),
          description +
            (lcs.sortBy(d => (d.iLeft, d.iRight)) zip expected.sortBy(d => (d.iLeft, d.iRight))).filter {
              case (l, r) => l != r
            }.mkString("\n")
        )
  ...
  test("Chunking sequence diffs") {
    ...
    cases.foreach {
      case (description, left, right, expectedDiffs) =>
        List(1, 2, 5, 100).zipWithIndex.foreach {
          case (chunkSize, i) => {
            val stringRep = StringTableDiff.valueDiffRenderer(
              E(left, right),
            ...
            assert(
              stringRep === expectedDiff,
              s"DiffStrings $description chunk size $chunkSize\n" +
  ...
  test("diff reports") {
    val cases = L(
      ...
      {
        val diffReport = ReportContent(
          L(L(D("a"), D("d")), L(E("b", "b1"), D("d"))),
          L(L(D("Col1"), D("Col2"), E("Col3", n), E(n, "Col4")), L(D("one"), D("two"), E("three", n), E(n, "three"))),
          L(L(E(7, 8), D(5), E(2, n), E(n, 23)), L(D(33), D(1), E(34, n), E(n, 34)))
        )
        (
          leftReport,
          rightReport,
          ...
          Some(
            ReportContent(
              L(L(D("a"), D("d")), L(E("b", "b1"), D("d"))),
              L(L(D("Col1"), E("Col3", n), E(n, "Col4")), L(D("one"), E("three", n), E(n, "three"))),
              L(L(E(7, 8), E(2, n), E(n, 23)), L(D(33), E(34, n), E(n, 34)))
            )
          )
        )
      },
      (
        ...
        leftReport,
        ReportContent(
          L(L(D("a"), D("d")), L(D("b"), D("d"))),
          L(L(D("Col1"), D("Col2"), D("Col3")), L(D("one"), D("two"), D("three"))),
          L(L(D(7), D(5), D(2)), L(D(33), D(1), D(34)))
        ),
        Some(emptyReport[ValueDiff[Any]])
      ),
      (
        leftReport,
        bitDifferentLeftReport,
        ReportContent(
          L(L(D("a"), D("d")), L(D("b"), D("d"))),
          L(L(D("Col1"), D("Col2"), D("Col3")), L(D("one"), D("two"), D("three"))),
          L(L(D(7), E(5, 6), D(2)), L(D(33), E(1, 2), D(34)))
        ),
        Some(
          ReportContent(
            L(L(D("a"), D("d")), L(D("b"), D("d"))),
      ...
      (
        leftReport,
        changeColumnAndRowHeaderReport,
        ReportContent(
          L(L(D("a"), D("d")), L(D("b"), E("d", "z"))),
          L(L(D("Col1"), D("Col2"), E("Col3", "Colz")), L(D("one"), D("two"), D("three"))),
          L(L(D(7), D(5), D(2)), L(D(33), D(1), D(34)))
        ),
        Some(ReportContent(L(L(D("b"), E("d", "z"))), L(L(E("Col3", "Colz")), L(D("three"))), L(L(D(34)))))
      ),
      (
        leftReport,
        extraColReport,
        ReportContent(
          L(L(D("a"), D("d")), L(D("b"), D("d"))),
          L(L(D("Col1"), D("Col2"), E(n, "Col2.5"), D("Col3")), L(D("one"), D("two"), E(n, "two.5"), D("three"))),
          L(L(D(7), D(5), E(n, 2), D(2)), L(D(33), D(1), E(n, 2), D(34)))
        ),
        Some(
          ReportContent(
            L(L(D("a"), D("d")), L(D("b"), D("d"))),
            L(L(E(n, "Col2.5")), L(E(n, "two.5"))),
            L(L(E(n, 2)), L(E(n, 2)))
          )
        )
      ),
      (
        ...
        extraRowReport,
        ReportContent(
          L(L(D("a"), D("d")), L(E(n, "z"), E(n, "y")), L(D("b"), D("d"))),
          L(L(D("Col1"), D("Col2"), D("Col3")), L(D("one"), D("two"), D("three"))),
          L(L(D(7), D(5), D(2)), L(E(n, 2), E(n, 3), E(n, 4)), L(D(33), D(1), D(34)))
        ),
        Some(
          ReportContent(
            L(L(E(n, "z"), E(n, "y"))),
        ...
        val diffReport =
          ReportContent(
            L(L(E("a", n), E("d", n)), L(E("b", n), E("d", n))),
            L(L(E("Col1", n), E("Col2", n), E("Col3", n)), L(E("one", n), E("two", n), E("three", n))),
            L(L(E(7, n), E(5, n), E(2, n)), L(E(33, n), E(1, n), E(34, n)))
          )
        (leftReport, emptyTestReport, diffReport, Some(diffReport))
      }, {
        val diffReport =
          ReportContent(
            L(L(D("a"), E("d", n)), L(E("b", n), E("d", n)), L(E(n, "b"), E(n, "d"), E(n, "c"))),
            L(L(D("Col1"), E("Col2", n), E("Col3", n), E(n, "Col2")), L(D("one"), E("two", n), E("three", n))),
            L(
              L(D(7), E(5, n), E(2, n), E(n, 5), E(n, 2)),
        ...
        (
          leftReport,
          unexpectedShapedReport,
          diffReport,
          Some(
            ReportContent(
              L(L(D("a"), E("d", n)), L(E("b", n), E("d", n)), L(E(n, "b"), E(n, "d"), E(n, "c"))),
              L(L(D("Col1"), E("Col2", n), E("Col3", n), E(n, "Col2"), D(n)), L(D("one"), E("two", n), E("three", n))),
              L(
                L(D(7), E(5, n), E(2, n), E(n, 5), E(n, 2)),
                L(E(33, n), E(1, n), E(34, n), D(n), D(n)),
                L(E(n, 33), D(n), D(n), D(n), D(n))
      ...
      (
        ...
        ReportContent(
          ...
          rightReport.mainData.map(_.map(E(n, _)))
        ),
        n
      ),
      (
        ...
        ReportContent(
          ...
          leftReport.mainData.map(_.map(E(_, n)))
        ),
        n
      ),
      (emptyTestReport, emptyTestReport, emptyReport[ValueDiff[Any]], Some(emptyReport[ValueDiff[Any]])), {
        val diffReport =
          ReportContent(L(L(D("a"), D("d")), L(E("b", n), E("d", n)), L(E(n, "b1"), E(n, "d"))), L(), L(L(), L(), L()))
        (
          ReportContent(leftReport.rowHeaders, L(L()), L(L())),
          ReportContent(rightReport.rowHeaders, L(L()), L(L())),
          diffReport,
          Some(ReportContent(L(L(E("b", n), E("d", n)), L(E(n, "b1"), E(n, "d"))), L(), L()))
        )
      }
    )
    cases.foreach {
  ...
  test("chunking diff reports") {
    val cases = L(
      ...
      (
        ...
        ReportContent(Array("No", "1", "2").map(Array(_)), 1, 1),
        List("""+-----+
          ||No   |
          |+-----+
          ||[-0-]|
      ...
      (
        ...
        ReportContent(Array("No", "1", "2", "3", "4", "5").map(Array(_)), 1, 1),
        List(
          """+-----+
          ||No   |
          |+-----+
  ...
  test("Section Diffs") {
    val cases = List(
      (L(L("A"), L("C")), L(L("A"), L("C")), L(L(D("A")), L(D("C")))),
      (L(L("A"), L("C")), L(L("D"), L("E")), L(L(E("A", "D")), L(E("C", "E")))),
